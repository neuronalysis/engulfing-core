...
class Comparator {
	...
	function compareTwoObjects($a,$b, $ignoreID = true, $convertNumericStrings = true, $referer) {
		if(is_object($a) && is_object($b)) {
			if(get_class($a)!=get_class($b))
				return false;
				foreach($a as $key => $val) {
				    if(!$this->compareTwoObjects($val,$b->$key, true, true, $key) && (!in_array($key, array("id")) || !$ignoreID)) {
			...
			while(!is_null(key($a)) && !is_null(key($b))) {
			    if (key($a)!==key($b) || !$this->compareTwoObjects(current($a),current($b), true, true, $referer))
					return false;
					next($a); next($b);
			}
