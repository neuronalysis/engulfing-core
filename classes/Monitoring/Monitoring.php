...
class Monitoring {
	...
	function getAccessPermissionByClientAndScope($userID, $clientIP, $scope) {
		...
		if (isset($userID)) {
		    $objects = $rest->orm->getByNamedFieldValues(new ORM_Request("Request", array("UserID" => $userID, "url" => "/api" . $scope, "sentAt" => date("Y-m-d"))));
			
			if (count($objects) < 500) return true;
		} else {
		    $objects = $rest->orm->getByNamedFieldValues(new ORM_Request("Request", array("clientIP" => $clientIP, "url" => "/api" . $scope, "sentAt" => date("Y-m-d"))));
