...
class CloudConvert {
    ...
    public function uploadByFTP($ftpRequest) {
    	$post_json = '{
    input: {
        ftp : {
            host: "ftp.engulfing.com",
            user: "dpr@engulfing.com",
            password: "orkpower78"
        }
    },
    file: "zkb_warrant_call.pdf",
    filename: "/zkb_warrant_call.pdf",
    outputformat: "html",
    output: {
        ftp : {
            host: "ftp.engulfing.com",
    ...
    function convertPDFtoHTML($url, $processing_file, $format, $location) {
    	$apikey = "3eBlFsNNQqunQAGT3lDRYOSkP2P4KUXTNrGQXn_C8hJEEe0L9Di-FVkNl8c5cpLrwUszNT6qZAkfQnzUITCQPA";
    
    	$process = $this->createProcess("pdf", "html", $apikey);
    	$process-> uploadByURL($url, $processing_file, "html" );
    
    	if ($process-> waitForConversion()) {
    		$process -> download(str_replace(".pdf", ".html", $location . $processing_file));
    		$process -> delete();
    	} else {
   		...
   		$files = file(str_replace(".pdf", ".html", $location . $processing_file));
   	
   		$new_file = array();
   		foreach ($files as $line) {
   			if (substr($line, 0, 10) == "@font-face") {
   			} else {
   				$new_file[] = $line; // push line new array
   			}
   		}
   	
   		file_put_contents(str_replace(".pdf", ".html", $location . $processing_file), $new_file);
    	
    	$content = file_get_contents(str_replace(".pdf", ".html", $location . $processing_file));
