<?php
$config = json_decode(file_get_contents(__DIR__ . "/../../../../config/neuronalysis-config.json"), true);

require_once $config['framework']['path'] . 'engulfing-core/vendor/autoload.php';
require_once $config['framework']['path'] . 'engulfing-extensions/vendor/autoload.php';

$tst = new Testing();
$tst->setConfig($config);

$config_dev = json_decode(file_get_contents($config['frontend']['path'] . "/config/dev-config.json"), true);
$config_live = json_decode(file_get_contents($config['frontend']['path'] . "/config/live-config.json"), true);
